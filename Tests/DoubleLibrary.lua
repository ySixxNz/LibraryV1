...
local DoubleColumns = setmetatable({}, { __mode = "k" })

local function GetDoubleColumns()
    local existing = DoubleColumns[Container]
    if existing then
        return existing
    end
    ...
    local state = { Holder = ColumnsHolder, Left = LeftColumn, Right = RightColumn, UseLeft = true }
    DoubleColumns[Container] = state
    return state
end
...
function ElementFunction:AddSection(SectionConfig)
    ...
    return CreateSection(SectionConfig, FullFrame)
end

function ElementFunction:AddDoubleSection(SectionConfig)
    local columns = GetDoubleColumns()
    local targetColumn = columns.UseLeft and columns.Left or columns.Right
    columns.UseLeft = not columns.UseLeft

    return CreateSection(SectionConfig, targetColumn)
end
