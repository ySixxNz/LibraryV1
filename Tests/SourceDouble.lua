...
local PendingDoubleRow = nil
local PendingDoubleGap = 10

function ElementFunction:AddDoubleSection(SectionConfig, Config)
    Config = Config or {}
    local gap = Config.Gap or PendingDoubleGap

    if not PendingDoubleRow then
        ...
        MakeElement("List", 0, 6).Parent = ColFrame

        PendingDoubleRow = {Row = RowFrame, Gap = gap}
        return CreateSection(SectionConfig, ColFrame)
    else
        local ColFrame = SetProps(
            MakeElement("TFrame"),
            {
                Size = UDim2.new(0.5, -(PendingDoubleRow.Gap / 2), 0, 0),
                AutomaticSize = Enum.AutomaticSize.Y,
                Parent = PendingDoubleRow.Row,
                ClipsDescendants = false,
                Name = "Column"
            }
        )
        MakeElement("List", 0, 6).Parent = ColFrame
        PendingDoubleRow = nil
        return CreateSection(SectionConfig, ColFrame)
    end
end
